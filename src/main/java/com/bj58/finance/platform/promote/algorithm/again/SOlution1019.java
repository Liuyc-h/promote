...
public class SOlution1019 {
    ...
    public int[] nextLargerNodes(ListNode head) {
        ...
        while (flowNode != null) {
            indexMap.put(index++, flowNode.value);
            flowNode = flowNode.next;
        }
        //定义一个栈结构
        Deque<Integer> deque = new LinkedList<>();
        //结果数组
        int[] result = new int[indexMap.size()];
        //流转索引值
        int flowIndex = 0;
        ...
        while (flowNode1 != null) {
            ...
            while (!deque.isEmpty()) {
                ...
                int prevIndex = deque.peek();
                if (indexMap.get(prevIndex) < flowNode1.value) {
                    result[prevIndex] = flowNode1.value;
                    deque.pop();
                }else{
                    break;
                }
