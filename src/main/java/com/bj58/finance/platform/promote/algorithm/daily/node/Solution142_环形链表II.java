...
public class Solution142_环形链表II {
    ...
    public ListNode detectCycle(ListNode head) {
        ...
        //获取环中的一点
        ListNode oneCicleNode = getOneCircleNode(head);
        if(oneCicleNode == null){
            return null;
        }
        //获取环的长度
        int circleLength = 1;
        ListNode flowNode = oneCicleNode;
        flowNode = flowNode.next;
        while(flowNode != oneCicleNode){
            flowNode = flowNode.next;
            circleLength = circleLength + 1;
        }
    ...
    //获取环中的一个点
    private ListNode getOneCircleNode(ListNode head){
        //快慢指针
        ListNode quickNode = head;
        ListNode slowNode = head;
